...
def adduser(data: dict): # TODO: Add try to outer, and integretyerror checks to see if the uuid, username, or email is duplicated - returning a diff num for each. However, the script that calls on this (will be routers/auth.py) should query the user (via database.py scripts) by uuid, username, and email first before the account gets added.
    # TODO: Appearently, the safety clauses were STILL not enough, and there are still some errors somewhere. Add "session.rollback()" and stuff
    if not all(k in data for k in ("username", "email", "password")):
        log("Tried to run adduser, but one or more of vars username, email, and password were missing.", level=6, perm=True)
        return 1
    ...
    with Session(engine) as session: # Auto opens and closes when the block is finished
        ...
        try:
            ...
            log(f"SQLite: ABORTED - Exception (likely IntegrityError) while trying to add UUID {data['uuid']}.", level=7, perm=True)
            session.rollback()
            return 1
        ...
        try: # 5. Set the UserNum #TODO
            ...
            while (i < 3):
                try:
                    ...
                    i += 1
                    # But before restarting, rollback the session for safety
                    session.rollback()
